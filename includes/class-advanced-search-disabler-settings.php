...
class Advanced_Search_Disabler_Settings
{
    ...
    public function __construct(string $plugin_name)
    {
        ...
        $this->disabler_mode = ['full_page', 'logged_out_users', 'exclude_post_ids'];
    }
    ...
    {
        register_setting($this->plugin_name, 'advanced_search_disabler_option', [$this, 'sanitization_callback_options_page']);

        add_settings_section(
            'advanced_search_disabler_main_section',
            __('Default Settings', $this->plugin_name),
            [$this, 'render_sections_field'],
            'advanced-search-disabler-settings-page'
        );

        add_settings_field(
            'advanced_search_disabler_mode_option',
            __('Deactivate search for', $this->plugin_name),
            [$this, 'render_disabler_mode_radio_field'],
            'advanced-search-disabler-settings-page',
            'advanced_search_disabler_main_section'
        );
        ...
        add_settings_field(
            ...
            __('Excluded page IDs', $this->plugin_name),
            [$this, 'render_disabler_exclude_post_ids_text_field'],
            'advanced-search-disabler-settings-page',
            'advanced_search_disabler_main_section'
        );
    }
    ...
    public function render_sections_field(): void
    {
        ...
        echo '<p class="description">' . __('Select your preferred options for the Advanced Search Disabler.', $this->plugin_name) . '</p>';
    }
    ...
    public function render_disabler_mode_radio_field(): void
    {
        ...
        echo '<input type="radio" name="advanced_search_disabler_option[disabler_mode]" value="full_page" ' . checked('full_page', $radio_option, false) . '> ' . __('Entire website', $this->plugin_name) . '<br>';
        echo '<input type="radio" name="advanced_search_disabler_option[disabler_mode]" value="logged_out_users" ' . checked('logged_out_users', $radio_option, false) . '> ' . __('For logged out users', $this->plugin_name) . '<br>';
        echo '<input type="radio" name="advanced_search_disabler_option[disabler_mode]" value="exclude_post_ids" ' . checked('exclude_post_ids', $radio_option, false) . '> ' . __('Exclude post IDs', $this->plugin_name) . '<br>';
    }
    ...
    public function render_disabler_exclude_post_ids_text_field(): void
    {
        ...
        echo '<p class="description">' . __('Here you can enter the post IDs you want to exclude from the search. Make sure to select the Exclude post IDs option at the top.', $this->plugin_name) . '</p>';
    ...
    public function sanitization_callback_options_page($input): array
    {
        ...
        if (!empty($input['disabler_mode']) && in_array($input['disabler_mode'], $this->disabler_mode)) {
